...
module uart_cmd_iic(
    ...
    reg [7:0] data_str [22:0];
    always@(posedge Clk)
    if(rx_done)begin
		
        data_str[22] <= #1 rx_data;
		data_str[21] <= #1 data_str[22];
    ...
    if(!Reset_n) begin
        ...
        if((data_str[0] == 8'h55) && (data_str[1] == 8'hA5) && (data_str[22] == 8'hF0))begin
            data <= #1 {data_str[6],data_str[7],data_str[8],data_str[9],
                        data_str[10],data_str[11],data_str[12],data_str[13],data_str[14],data_str[15],
                        data_str[16],data_str[17],data_str[18],data_str[19],data_str[20],data_str[21]};
            num_cmd <= #1 data_str[5];
            address <= #1 {data_str[3],data_str[4]};
			device_id <= #1 data_str[2];
            cmdvalid <= #1 1;
        end
    end
    else
